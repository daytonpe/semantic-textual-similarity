...
def pretty_print(report):
    ...
    for label in report.keys():
        if label == 'micro':
            print
        print(str(label) + "\t" + str(report[label]['precision']) + "\t" + str(
            report[label]['recall']) + "\t" + str(report[label]['fscore']))
...
if __name__ == "__main__":
    ...
    # for i in range(0, len(predicted_tags.keys())):
    #     print(gold_tags.keys()[i], predicted_tags.keys()[
    #           i], (gold_tags.keys()[i], predicted_tags.keys()[i]))
    try:
