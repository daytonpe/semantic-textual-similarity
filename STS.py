...
def readData():
    ...
    for sentence in text.split('\n'):
        ...
        first_sentence.insert(len(first_sentence),(sentence.split('\t')[1].lower()).rstrip('.'))
        second_sentence.insert(len(first_sentence),(sentence.split('\t')[2].lower()).rstrip('.'))
...
def preprocess():
    ...
    for sentence in first_sentence:
        ...
        first_sentence_tags.insert(len(first_sentence_tags), nltk.pos_tag(tokens))
    ...
    for sentence in second_sentence:
        ...
        second_sentence_tags.insert(len(second_sentence_tags), nltk.pos_tag(tokens))
    ...
    for sentence in first_sentence_tokens:
        ...
        first_sentence_lemmas.insert(len(first_sentence_lemmas), sentence_components)
    ...
    for sentence in second_sentence_tokens:
        ...
        for token in sentence:
            lemmas = lemmatizer.lemmatize(token)
            sentence_components.insert(len(sentence_components), lemmas)
        second_sentence_lemmas.insert(len(second_sentence_lemmas), sentence_components)
